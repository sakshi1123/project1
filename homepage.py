...
'''def View():
    ...
    a_rows = cur.fetchall()
    print('1):', a_rows)
    cur.execute('SELECT * FROM addfinal_course WHERE CNAME="bcom"'.\
                format(addfinal_course=t_n, CNAME=c2))
    a_rows = cur.fetchall()
    print('1):', a_rows)'''
...
comMenu = Menu(menu)
menu.add_cascade(label="  COMPUTER APPLICATION   ", menu=commerceMenu)
...
MyButton1 = ttk.Button(root, text='Search', width=10).pack(padx=5,pady=5)
entry1.bind('<Return>')

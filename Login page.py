...
class Main:
    ...
    def login(self):
         ...
         if results:
             ...
             self.head['pady'] = 150
              ...
              messagebox.showerror("-- ERROR --","Oops!! username not matched", icon="error")
    ...
    def new_user(self):
        ...
        if cursur.fetchall():
            messagebox.showerror("-- ERROR --","username already exists", icon="error")
        else:

            messagebox.showerror("-- COMPLETE --","account created successfully!!", icon="info")
            self.log()
    ...
    def widgets(self):
        self.head = Label(self.master,text = " LOGIN TO COLLEGOPEDIA",font = ('freesansbold',25),pady=40)
        self.head.pack()
